...
(defun collect-asd-component-files (asd-path)
  ...
  (handler-case
      (let* (;; Ensure ASDF package exists so ASDF:DEFSYSTEM can be read BEFORE parsing.
             ...
             (files nil))
        ;; Parse forms directly instead of using cached globals
        (let ((parse-context (read-forms-with-full-positions content)))
          (when *verbose*
            (logf "; Parse trees found: ~D~%" (length (parse-context-parse-trees parse-context)))
            (dolist (tree (parse-context-parse-trees parse-context))
              (logf ";   Tree: ~S~%" tree)))
          (let ((forms (read-top-forms-with-pos-ctx parse-context)))
              (logf "; Reading ASD ~A: ~D toplevel form(s)~%" asd-path (length forms))
              (when *verbose*
                (dolist (pair forms)
                  (let ((f (first pair)))
                    (when (consp f)
                      (logf ";  form head: ~A~%" (first f))))))
