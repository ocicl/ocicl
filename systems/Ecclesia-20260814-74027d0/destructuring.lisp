       ...
       ,@(if key-p
		 ...
		 `((when (and (> ,arg-count-var
				 ,(+ required-count
				     optional-count))
			      (oddp ,arg-count-var))
		     (error "odd number of keyword arguments supplied")))
		 `((when (and (> ,arg-count-var
				 ,(+ required-count
				     optional-count))
			      (evenp ,arg-count-var))
...
;;; Given a function name, return the name of the implicit block that
;;; the macro function establishes.
(defun function-block-name (function-name)
