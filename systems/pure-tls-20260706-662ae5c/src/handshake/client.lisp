...
(defun process-hello-retry-request (hs server-hello hrr-raw-bytes)
  ...
  (let ((extensions (server-hello-extensions server-hello)))
    ...
    ;; Note: Unsolicited ECH extensions are rejected in the extension allowlist above
  ...
  (let* ((server-hello (handshake-message-body message))
    ...
    (unless (zerop (server-hello-legacy-compression-method server-hello))
      ...
      (error 'tls-handshake-error
             ...
             :state :wait-server-hello))
    ;; CL-SEC-2026-0113: RFC 8446 Section 4.1.3: legacy_session_id_echo MUST match
    (let ((sent-id (client-handshake-legacy-session-id hs))
      ...
      (unless (equalp (or sent-id #()) (or echoed-id #()))
        ...
        (error 'tls-handshake-error
               ...
               :state :wait-server-hello)))
    ;; CL-SEC-2026-0112: RFC 8446 Section 4.1.3: TLS 1.3 downgrade sentinel check
                ...
                (when (= (client-handshake-verify-mode hs) +verify-required+)
                  (let ((ticket (client-handshake-offered-psk hs)))
                    (unless (and (client-handshake-psk-accepted hs)
                                 ticket
                                 (session-ticket-verified-hostname ticket)
                    ...
                    (setf (client-handshake-verified-hostname hs)
                          (session-ticket-verified-hostname ticket))))
