  ...
  not be seen." form)
      ...
      (t ;; Lisp function call
       (return-code-modifying-body #'walk-arglist (cdr form)
				   #L(list (recycle-code (cons (car form) !1) form))))))
...
(defun recycle-code (form original-form)
  (if (equal form original-form) original-form form))
  ...
  (cond
   ...
   ((symbolp (car form))
	...
	((catch if locally multiple-value-call multiple-value-prog1
	  progn progv setq throw unwind-protect)
	 (free-vars-list (cdr form) bound-vars))
        ((tagbody)
	 (free-vars-list (remove-if #'symbolp (cdr form)) bound-vars))
...
(defun return-seq-code (&key element-var sequence access-fn size-fn
  ...
  (let* ((index-var-spec (or with-index (genvar 'index)))
	 ...
	 (seq-var (if (or access-fn (not (symbolp sequence)))
		      (make-var-and-binding 'sequence sequence
					    :type sequence-type)))
	 (seq-code (or seq-var sequence))
	 (step-var (if (not (constant? by))
		       (make-var-and-binding 'step by :type 'fixnum)))
	 (step (or step-var by))
	 ...
	 (limit-var (if (not (numberp limit-value))
			(make-var-and-binding 'limit limit-value :type 'fixnum)))
    ...
    (make-binding index-var-spec initial-value :type 'fixnum)
...
(defclause (repeat n)
  ...
  (let* ((c-type (or (expression-type n) 'fixnum))
	 (count-var (make-var-and-binding 'count n :type c-type)))
    (setq *loop-end-used?* t)
    (return-code :body `((if (<= ,count-var 0) (go ,*loop-end*)))
...
(defclause-driver (for var-spec &sequence)
  ...
  (let* ((var (extract-var var-spec))
    ...
    (make-binding var-spec init-val :type 'number)
    (return-driver-code :next next
			:variable var)))
...
(defclause-driver (for var on list &optional by (step ''cdr))
  "Sublists of a list"
  (top-level-check)
  (let* ((list-var (make-var-and-binding 'list list))
    ...
    (setq *loop-end-used?* t)
    (return-driver-code :next (list test
				    setqs
...
(defclause-driver (for var in list &optional by (step ''cdr))
  "Elements of a list"
  (top-level-check)
  (let* ((on-var (make-var-and-binding 'list list :type 'list))
...
;;; (FOR IN-HASH-TABLE) and (FOR IN-HASHTABLE)
;;; 2023-12-29 make in-hash-table and in-hashtable synonyms, since
;;;            the former matches CL usage, etc, but the latter
;;;            undoubtedly has existing code depending upon it
;;; Note that this funky macro is so that the two versions have the same body,
;;; rather than cloning it and risking them getting out of sync.
(defmacro define-hash-table-driver (driver-word)
  `(defclause-driver (for key-val-vars ,driver-word table)
     "Elements and keys of a hash-table"
     (top-level-check)
...
(define-hash-table-driver in-hash-table)

(define-hash-table-driver in-hashtable)
...
(defun return-stream-driver-code (var thing reader stream-or-file generate)
  (let* ((evar (extract-var var))
	 ...
	 (stream-var (make-var-and-binding
                      'stream (if (eq stream-or-file :file)
