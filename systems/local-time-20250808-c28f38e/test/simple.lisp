...
(deftest test/sunday?-monday?-etc ()
  (let ((*default-timezone* +utc-zone+))
    (is (sunday? (make-timestamp :day 9083))) ;Jan 12 2025
    (is (monday? (make-timestamp :day 9084)))
    (is (tuesday? (make-timestamp :day 9085)))
    (is (wednesday? (make-timestamp :day 9086)))
    (is (thursday? (make-timestamp :day 9087)))
    (is (friday? (make-timestamp :day 9088)))
    (is (saturday? (make-timestamp :day 9089)))
    (is (not (sunday? (make-timestamp :day 9084))))
    (is (not (monday? (make-timestamp :day 9083))))
    (is (not (saturday? (make-timestamp :day 9084))))
    (is (not (friday? (make-timestamp :day 9083))))
    (is (not (friday? (make-timestamp :day 9084))))
    (is (not (wednesday? (make-timestamp :day 9085))))
    (is (not (thursday? (make-timestamp :day 9085))))
    (is (not (thursday? (make-timestamp :day 9083))))
    (is (not (saturday? (make-timestamp :day 9085))))
    (is (not (sunday? (make-timestamp :day 9088))))))
