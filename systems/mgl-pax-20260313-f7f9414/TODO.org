...
** MAYBE transcribe debugger sessions
... e.g. where a restart is selected.
** MAYBE support trailing comments in values
E.g.
*print-level*
=> NIL ; turned off
* elisp
** MAYBE support SLY?
https://github.com/melisgl/mgl-pax/issues/37
...
- Untangled Literate Programming (https://quotenil.com/untangling-literate-programming.html)
** Pitch for Lispers
...
** vs notebooks
No, notebooks are not LP. They are execution transcripts.

Notebooks have the strict order problem of LP without the chunking
tools. Thus, the linear flow they require is driven more by the
necessities of code than by presentation. Their "IDE" sucks. They have
a different focus/purpose: the transcript.
** proximity
Both humans and AI can check code against its documentation more
easily if the corresponding pieces are near each other.
** overlap between info in doc and code: redundancy for striving for correctness
** extra information in doc relative to code: might be useful when changing the code
